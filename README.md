...
- 🔍 **环境检测** — 启动时自动检测 Chrome 浏览器，未安装则提示下载
- 🔐 **自动登录** — 输入账号密码，自动完成登录流程
- 📝 **全自动答题** — 自动处理单选、多选、文本题，一个不落
- 🎯 **智能选优** — 优先选择"无上述问题"选项，文本题随机填入正面评语
- 🔄 **多栏目支持** — 常规评教、课程思政评教、专业认证评教全覆盖
- 📄 **自动翻页** — 评教列表有多页时自动翻页处理，不遗漏任何问卷
- 🛡️ **异常处理** — 自动跳过已评教但仍显示「待评价」的异常问卷
- 🖥️ **开箱即用** — 提供 exe 可执行文件，无需 Python 环境即可运行
...
- 运行前会自动检测 Chrome 浏览器，未安装会提示下载地址
